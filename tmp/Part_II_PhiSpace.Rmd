...
# Some customised functions for this case study
source("utils.R")
```
...
```{r}
load("~/Dropbox/MIG_Spatial_Workshop/data/ref_luca.rda")
lung5 <- readRDS("~/Dropbox/MIG_Spatial_Workshop/data/lung5_normalised.rds")
```
...
```{r}
ref_luca <- logTransf(ref_luca)
lung5 <- logTransf(lung5)
ref_luca <- scranTransf(ref_luca)
# lung5 already normalised by SpaNorm
```
...
```{r}
lung5 <- PhiSpace(
  reference = ref_luca, query = lung5,
  phenotypes = YtrainName, refAssay = "logcounts", regMethod = "PLS"
)
reducedDim(lung5, "PhiSpace")[1:5,1:5]
```

The first output of PhiSpace is a spatial unit by cell type matrix deposited in `reducedDim(lung5, "PhiSpace")`. It's a `reducedDim` object since we view cell state characterisation as *dimension reduction* from the *omics space* to the **phenotype space**. That is, after PhiSpace, the cell state of each cell is characterised by (dozens of) cell types in the reference, instead of expression levels of (tens of thousands of) genes.

All our downstream analyses will be based on `reducedDim(lung5, "PhiSpace")`, which we simply refer to as PhiSpace scores.
...
```{r eval=FALSE}
tempSavePlots(lung5, tissueName = "lung5_sub", freeColScale = T, censQuant = 0.5, psize = 0.8)
```
...
```{r eval=FALSE}
sc <- readRDS("~/Dropbox/MIG_Spatial_Workshop/output/Lung5_sub_PhiSpace.rds")
reducedDim(lung5, "Phi5Ref") <- sc
tempSavePlots(lung5, tissueName = "lung5_sub", methodName = "Phi5Ref", freeColScale = T, censQuant = 0.5, psize = 0.8)
```

Delete data objects to free up RAM.

```{r}
rm(sc, lung5, ref_luca, p); gc()
```

[Total memory usage at this point: 1.62 GiB]
...
```{r}
allTissues <- readRDS("~/Dropbox/MIG_Spatial_Workshop/output/PhiSpaceResSPE_0.5subsample.rds")
saveRDS(allTissues[1:3], "~/Dropbox/MIG_Spatial_Workshop/output/PhiSpaceResSPE_0.5subsample1.rds")
saveRDS(allTissues[4:5], "~/Dropbox/MIG_Spatial_Workshop/output/PhiSpaceResSPE_0.5subsample2.rds")
saveRDS(allTissues[6:8], "~/Dropbox/MIG_Spatial_Workshop/output/PhiSpaceResSPE_0.5subsample3.rds")
```

Each spe object contains empty gene expression (NB the dimension is 1 by 43296).
...
VizSpatial(allTissues[["Lung5_Rep2"]], groupBy = "total", ptSize = 1, reOrder = T)
VizSpatial(allTissues[["Lung5_Rep3"]], groupBy = "total", ptSize = 1, reOrder = T)
```
...
VizSpatial(
  allTissues[["Lung5_Rep2"]], ptSize = 0.5, groupBy = "region", legend.position = "bottom", legend.symb.size = 2
) 
VizSpatial(
  allTissues[["Lung5_Rep3"]], ptSize = 0.5, groupBy = "region", legend.position = "bottom", legend.symb.size = 2
) 
```
...
```{r eval=FALSE}
query <- readRDS("~/Dropbox/MIG_Spatial_Workshop/data/Lung5Rep1_0.5sub.rds")
# Banksy parameters
lambda = 0.2
npcs = 50
k_geom = 30
...
query = computeBanksy(query, assay_name = "logcounts", k_geom = k_geom)
# compute PCA on the features
set.seed(1000)
query = runBanksyPCA(query, lambda = lambda, npcs = npcs)
# perform graph-based clusering
set.seed(1000)
query = clusterBanksy(query, lambda = lambda, npcs = npcs, resolution = res)
allTissues[["Lung5_Rep1"]]$BanksyClust <- query$clust_M0_lam0.2_k50_res0.3
rm(query); gc()
```
...
```{r fig.asp=1}
tempClustCols <- c(
  "1" = "#B15928", "2" = "#1F78B4", "3" = "black", 
  "4" = "#33A02C", "5" = "gray50", "6" =  "#FFFF99", 
  "7" = "#A6CEE3" , "8" =  "#FF7F00", "9" = "#E41A1C"
)
...
Comparing them to the pathological niche annotation, we can see, for example, the tumour niche was further clustered into two sub-niches, one black, on gray; and the stroma was further clustered into several sub-niches. What are they? We can do a 'differential cell state ananlysis' based on PhiSpace scores of cells in each niche. Similar to differential gene expression analysis, this can be done in several ones. Here we use a supervised classification based approach. Namely we use PhiSpace scores of the cells to predict their Bansky cluster memberships. Any classification method will do, as long as they can rank the importance of features in predicting the cluster membership. Here we use PLS:
...
`impScores` is a cell type by cluster membership matrix, giving the
...
- Cluster 3 (black) as annotated as Tumour with strong mesothelial (possibly EMT), basal (stemness) and secretory cell (cell of origin) identities; cluster 5 (gray) had strong activated myofibroblast identity (CAF?).

- In stromal area, cluster 4 (green) had stronger endothelial identities; cluster 2 (blue) was myeloid-enriched.

- Cluster 1 (brown) might include lymphoid structures.
...
```{r fig.asp=1}
clustResPath <- "~/Dropbox/MIG_Spatial_Workshop/output/Lung5_Rep1_PhiClusts5Refs.rds"
if(!file.exists(clustResPath)){
  ...
  set.seed(94863)
  clust_res <- kmeans(mat2clust, centers = 9, iter.max = 200L, nstart = 50, algorithm = "Lloyd") 
  saveRDS(clust_res, clustResPath)
} else {
  clust_res <- readRDS(clustResPath)
}
query$PhiClustRaw <- as.character(clust_res$cluster)
...
query$PhiClust <- newClust
```

Now visualise the aligned PhiSpace clusters.
```{r fig.asp=1}
VizSpatial(query,  groupBy = "PhiClust", ptSize = 1, legend.position = "top", legend.symb.size = 2) + 
  scale_colour_manual(values = tempClustCols) 
```
...
```{r fig.asp=1}
clustResPath <- "~/Dropbox/MIG_Spatial_Workshop/output/Lung5_Rep1_PhiClusts5Refs_smoothed.rds"
if(!file.exists(clustResPath)){
  ...
  clust_res <- kmeans(mat2clust, centers = 9, iter.max = 500L, nstart = 50, algorithm = "Lloyd")
  saveRDS(clust_res, clustResPath)
} else {
  clust_res <- readRDS(clustResPath)
}
PhiClust_smooth <- as.character(clust_res$cluster)
# Align clusters
query$PhiClust_smooth <- align_clusters(PhiClust_smooth, query$BanksyClust)
VizSpatial(allTissues[["Lung5_Rep1"]], ptSize = 0.5, groupBy = "region", legend.position = "bottom", legend.symb.size = 2) 
VizSpatial(query,  groupBy = "PhiClust_smooth", ptSize = 1, legend.position = "bottom", legend.symb.size = 2) + 
  scale_colour_manual(values = tempClustCols) 
```
...
The first thing we notice (ie I noticed) is that the tumour region was also clustered into subclusters. Instead of the black--grey division as in Banksy, now we have this black--red--grey division. 

The second thing is that there were some interesting subclusters emerging in the stromal region, notably the yellow and the brown niches.
...
- Again we have a cluster (cluster 9, red) that's predominantly tumourous, mesothelial, basal and secretory. 

- The gray cluster is similar to the Banksy gray cluster, with fibroblast identities.

- This time we have yet the other tumour niche (cluster 3, black), which is somewhat similar to cluster 9 in Banksy result.

- Cluster 1 (brown) has a mixture of myeloid and lymphoid cells, and has this islandular structure, which is idiocyncratic of TLS.





### Optional: hoodscanR (TBC)

hoodscanR identifies niches based on discrete cell type labels of cells. 

Get cell types and apply hoodscanR.
```{r eval=FALSE}
...
set.seed(1000)
nb_cells = hoodscanR::findNearCells(query, k = 100, anno_col = "PhiCellType")
nb_cells$cells[1:5, 1:5]
nb_cells$distance[1:5, 1:5]
probmat = hoodscanR::scanHoods(nb_cells$distance)
dim(probmat)
...
query = hoodscanR::mergeHoodSpe(query, neighbourhoods)
# identify spatial domains based on neighbourhood composition (5 clusters)
set.seed(1000)
query = hoodscanR::clustByHood(query, pm_cols = colnames(neighbourhoods), k = 5, val_names = "hsR_clusters")
```

Visualisation. 
```{r fig.asp=1, eval=FALSE}
plotSpatial(query, colour = hsR_clusters, alpha = 0.75, size = 0.3) +
...
plotSpatial(query, colour = region, size = 0.3) +
...
First let's look at these tissues and the pathological annotation. They have very different niche compositions. Different 
```{r fig.asp=1}
...
knitr::kable(tab)
write.csv(tab, "figs/4Refs/signatures/Lung5_Rep1_tumour.csv")
```

Phenotypic signatures of individual tumours.

```{r}
tumTab <- do.call("cbind", tumourSig)[1:10,]
knitr::kable(tumTab)
# xtable::xtable(tumTab, type = "latex")
write.csv(tumTab, "figs/4Refs/signatures/allTumours.csv")
```


## Niches across samples
Identify niches across samples.
```{r}
if(F){
  allTissues <- lapply(
    allTissues, function(spe){
      spatialSmoother(spe, smoothReducedDim = T, kernel = "gaussian", k = 15)
    }
  )
  ...
  multiClustRes <- kmeans(mat2clust, centers = 9, iter.max = 500L, nstart = 20, algorithm = "Lloyd")
  saveRDS(multiClustRes$cluster, "/data/projects/punim0613/JiaDong/PhiSpace/MIG_workshop/output/multiPhiClustRes.rds")
} else {
  multiClustRes <- readRDS("/data/projects/punim0613/JiaDong/PhiSpace/MIG_workshop/output/multiPhiClustRes.rds")
}
```
...
for(ii in 1:length(allTissues)){
  spe <- allTissues[[ii]]
  spe$multiSampClust <- as.character(multiClustRes[colnames(spe)])
  allTissues[[ii]] <- spe
}
multiSampClustCols <- c(
  "1" = "#FF7F00", "2" = "gray50", "3" = "#E41A1C", "4" = "#1F78B4" , 
  "5" = "#A6CEE3", "6" =  "#33A02C", "7" = "#B15928", "8" = "black", "9" = "#FFFF99"
)
lapply(1:length(allTissues), function(ii){
  p <- VizSpatial(allTissues[[ii]], ptSize = 1, groupBy = "multiSampClust", legend.position = "bottom", legend.symb.size = 2) + scale_colour_manual(values = multiSampClustCols)
  return(p)
})
```

```{r}
saveRDS(allTissues, "/data/projects/punim0613/JiaDong/PhiSpace/MIG_workshop/output/all_tissue.rds")
names(allTissues)
multiSampClustCols <- c(
  "1" = "#FF7F00", "2" = "gray50", "3" = "#E41A1C", "4" = "#1F78B4" , 
  "5" = "#A6CEE3", "6" =  "#33A02C", "7" = "#B15928", "8" = "black", "9" = "#FFFF99"
)
nicheAnn <- c("1" = "Lymphoid", "2" = "Tumour/Stroma", "3" = "Stroma", "4" = "Immune/Stroma",
              "5" = "Macrophages", "6" = "Immune", "7" = "Tumour1", "8" = "Tumour2", "9" = "Plasmablast-stroma")
```
...
## Cell type colocalisation

Hypothesis: combinations of cell states in cancer cells are very unique, enough to distinguish them from other niche. First check if correlation matrices (vectorised) indeed cluster according to niche label.

Co-presence matrix of tumour interior niche of Lung5_Rep1, as an example.

```{r eval=FALSE}
tissueName <- "Lung5_Rep1"
PhiSc <- allSc[[tissueName]]$PhiSpace
colDat <- allSc[[tissueName]]$colData
coords <- allSc[[tissueName]]$coord

unique(colDat$region)
...
tempHeatmap(corMat, binColour = F, useSeriate = T)
...
Generate niche-specific co-localisation matrices. Only use luca reference
...
corMatVec <- lapply(
  ...
  function(ii){
    ...
    # Only use annotations from Luca reference
...
```{r fig.asp=1}
matrixPlot(pca_res$scores, comp_idx = 1:2, pointSize = 3, colBy = corMatVec$niche, manualCol = multiSampClustCols) 
...
loadBarplot(pca_res$loadings, "comp3", showNeg = F, absVal = T, nfeat = 10)
loadBarplot(pca_res$loadings, "comp4", showNeg = F, absVal = T, nfeat = 10)
```
...
# Binarise niches
nicheBinary <- as.numeric(corMatVec$niche %in% c("2", "7", "8"))
...
loadBarplot(dwd_res$loadings, "comp1", nfeat = 10)
```
