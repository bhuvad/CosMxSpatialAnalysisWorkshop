...
```{r fig.asp=1}
df <- lapply(1:length(allTissues), function(x) reducedDim(allTissues[[x]], "PhiSpace_smoothed"))
df <- do.call(rbind, df)
...
# Method 1: Faceted boxplots ordered by median within each cluster
boxplot_data %>%
  group_by(cluster) %>%
  mutate(cell_type_ordered = reorder(cell_type, median_score)) %>%
  ungroup() %>%
  ggplot(aes(x = cell_type_ordered, y = score, fill = cell_type_ordered)) +
