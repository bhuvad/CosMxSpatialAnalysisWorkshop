...
# usethis::use_data(allTissues1, overwrite = T)
# usethis::use_data(allTissues2, overwrite = T)
# usethis::use_data(allTissues3, overwrite = T)
...
tempClustCols <- c(
  ...
  "4" = "#33A02C", "5" = "#FFFF99", "6" =  "gray50", 
  "7" = "#A6CEE3" , "8" =  "#FF7F00", "9" = "#E41A1C", "10" = "#6A3D9A"
)
...
- Cluster 2 (black) as annotated as Tumour with strong mesothelial (possibly EMT), basal (stemness) and secretory cell (cell of origin) identities; cluster 6 (gray) had strong activated myofibroblast identity (CAF?).
...
set.seed(94863)
Lung5Rep1_PhiClusts <- kmeans(mat2clust, centers = 10, iter.max = 500L, nstart = 20, algorithm = "Lloyd") 
...
# usethis::use_data(Lung5Rep1_PhiClusts, overwrite = T)
...
Lung5Rep1_PhiSmoothClusts <- kmeans(mat2clust, centers = 10, iter.max = 500L, nstart = 20, algorithm = "Lloyd")
# saveRDS(Lung5Rep1_PhiSmoothClusts, "~/Dropbox/MIG_Spatial_Workshop/output/Lung5_Rep1_PhiClusts5Refs_smoothed.rds")
```
...
# usethis::use_data(Lung5Rep1_PhiSmoothClusts, overwrite = T)
...
query <- clustByHood(query, pm_cols = colnames(hoods), k = 10)
...
set.seed(1000)
query = clustByHood(query, pm_cols = colnames(neighbourhoods), k = 10, val_name = "hsR_clusters")
...
# usethis::use_data(multiClustRes,overwrite = T)
data("multiClustRes")
