...
author: 
  ...
  - name: Yifan Qin
    orcid: 0000-0002-9532-1119
    url: https://github.com/qunatic
    email: yifan.qin.1@unimelb.edu.au
    affiliation:
      - Melbourne Integrative Genomics (MIG), The University of Melbourne
output:
...
suppressPackageStartupMessages(library(vizOmics)) # Sister package of PhiSpace for visualisation
...
```{r eval=FALSE}
saveCellTypeMaps(lung5_norm, tissueName = "lung5_norm_sub", freeColScale = T, censQuant = 0.5, psize = 0.8)
```
...
saveCellTypeMaps(lung5_norm, tissueName = "lung5_norm_sub", methodName = "Phi5Ref", freeColScale = T, censQuant = 0.5, psize = 0.8)
```
...
```{r eval=FALSE}
saveCellTypeMaps(allTissues[["Lung5_Rep1"]], tissueName = "Lung5_Rep1", freeColScale = T, censQuant = 0.5, psize = 0.5)
```
...
```{r}
query <- allTissues[["Lung5_Rep1"]]
response <- query$BanksyClust
selectRes <- rankFeatures(query, response, method = "PLSDA", ncomp = length(unique(response)))
impScores <- selectRes$importance_scores
```
...
```{r fig.asp=1, eval=FALSE}
Lung5Rep1_PhiClusts <- clusterPhiSpace(query, k = 9, ncomp = 30, seed = 32123)
# save(Lung5Rep1_PhiClusts, file = "~/Dropbox/Research_projects/MIG_Spatial_Workshop/output/Lung5Rep1_PhiClusts.rda")
```
...
Lung5Rep1_PhiClusts
query$PhiClustRaw <- as.character(Lung5Rep1_PhiClusts$clusters)
...
```{r eval=FALSE}
Lung5Rep1_PhiSmoothClusts <- clusterPhiSpace(query, k = 9, ncomp = 30, seed = 32123, reducedDimName = "PhiSpace_smoothed")
# save(Lung5Rep1_PhiSmoothClusts, file = "~/Dropbox/Research_projects/MIG_Spatial_Workshop/output/Lung5Rep1_PhiSmoothClusts.rda")
```
...
PhiClust_smooth <- as.character(Lung5Rep1_PhiSmoothClusts$clusters)
# Align clusters
...
PhiSpaceProb <- Score2Prob(reducedDim(query,"PhiSpace"))
# hoods <- mergeByGroup(probmat, PhiSpaceProb, continuousAnnotation = T)
...
```{r fig.asp=1}
featSelRes <- rankFeatures(query, "PhiClust", method = "PLSDA", ncomp = length(unique(query$PhiClust)))
selectFeat(featSelRes$importance_scores)$orderedFeatMat[1:5,]
```
...
for(ii in 1:length(tumourNames)){
  ...
  regCoef <- rankFeatures(query, "region", ncomp = length(unique(query$region)))$importance_scores
...
multiClustRes <- clusterPhiSpace(mat2clust, k = 9, iter.max = 1000, seed = 83904)
# save(multiClustRes, file = "~/Dropbox/Research_projects/MIG_Spatial_Workshop/output/multiClustRes.rda")
```
...
for(ii in 1:length(allTissues)){
  spe <- allTissues[[ii]]
  spe$multiSampClust <- as.character(multiClustRes$clusters[colnames(spe)])
  allTissues[[ii]] <- spe
}
```
...
df <- do.call(rbind, df)
df <- as.data.frame(df) %>% mutate(cluster = as.character(multiClustRes$clusters))
plotGroups(
  df, "cluster", title = "Top 5 Cell Types per Cluster (Ordered by Median Score)",
...
plotCorrelation(corMat, seriation_method = "PCA")
...
ggplot(df, aes(x = comp1, y = comp2, colour = niche)) +
  ...
  theme_bw() + scale_colour_brewer(palette = "Set1") + xlim(-6.5, 7)

# Loading# Loadingniche
plotLoadings(pca_res$loadings, "comp1", showNeg = F, absVal = T, nfeat = 10)
plotLoadings(pca_res$loadings, "comp2", showNeg = F, absVal = T, nfeat = 10)
```
...
dwd_res <- rankFeatures(X, response = nicheBinary, method = "DWD")
```
...
```{r}
dwd_res$scores %>% as.data.frame() %>% mutate(niche = nicheBinaryByName) %>%
  ggplot() + geom_density(aes(DWD_score, fill = niche), alpha = 0.5) +
  theme_bw() + theme(legend.title = element_text(face ="bold")) + xlab("DWD score")
plotLoadings(dwd_res$importance_scores, "DWD_weight", nfeat = 20)
```
