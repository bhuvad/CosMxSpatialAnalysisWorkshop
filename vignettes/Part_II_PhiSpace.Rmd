...
selectRes <- correlatePhiSpace(query, response, method = "PLSDA", ncomp = length(unique(response)))
...
```{r fig.asp=1}
featSelRes <- correlatePhiSpace(query, "PhiClust", method = "PLSDA", ncomp = length(unique(query$PhiClust)))
...
for(ii in 1:length(tumourNames)){
  ...
  regCoef <- correlatePhiSpace(query, "region", ncomp = length(unique(query$region)))$importance_scores
