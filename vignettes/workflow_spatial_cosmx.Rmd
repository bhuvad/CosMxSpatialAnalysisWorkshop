...
lung5$cell_type = prediction$labels
```
...
set.seed(1000)
lung5 = clustByHood(lung5, pm_cols = colnames(neighbourhoods), k = 5, val_names = "hoodscanr_region")
# plot predictions
...
```{r}
plotProbDist(lung5, pm_cols = colnames(neighbourhoods), by_cluster = TRUE, plot_all = TRUE, show_clusters = as.character(seq(5)))
```
...
```{r}
# identify markers
marker.info = scoreMarkers(lung5, clusters, pairings = list(c("B cell_4", "T cell regulatory_4"), c("B cell_3", "T cell regulatory_3")))
marker.info
```
...
```{r fig.width=8, fig.height=8}
# study markers of T cell regulatory_4
chosen_tcell = marker.info[["T cell regulatory_4"]]
