...
First let's look at these tissues and the pathological annotation. They have very different niche compositions. 
```{r fig.asp=1}
...
if(F){
  ...
  saveRDS(multiClustRes, "~/Dropbox/MIG_Spatial_Workshop/output/multiPhiClustRes.rds")
} else {
  multiClustRes <- readRDS("~/Dropbox/MIG_Spatial_Workshop/output/multiPhiClustRes.rds")
}
...
multiSampClustCols <- c(
  "1" = "#FF7F00", "2" ="gray50", "3" =  "#FFFF99", "4" = "#A6CEE3", 
  "5" = "#1F78B4", "6" =  "#E41A1C", "7" = "#6A3D9A", "8" = "#B15928", "9" = "black", "10" = "#FB9A99", "11" = "#B2DF8A", "12" = "#33A02C"
)
lapply(1:length(allTissues), function(ii){
...
What are these niches? As usual, we can do a PLS analysis to see which cell types are enriched in each niche.
```{r}
XX <- lapply(1:length(allTissues), function(x) reducedDim(allTissues[[x]], "PhiSpace_smoothed")) 
XX <- do.call(rbind, XX)
YY <- codeY_vec(multiClustRes$cluster)
ncomp = ncol(YY)
plsRes <- PhiSpace:::mvr(XX, YY, ncomp = ncomp, method = "PLS")
impScores <- plsRes$coefficients[,,ncomp]
```

Recall that only lung6 was from LUSC patients and all other samples were from LUAD patients. Cluster 1 (black), a cancer niche common in lung5, didn't appear in lung6 at all. There are also niches that are more abundant, eg clusters 11 (purple) and 2 (red), that were more common in lung6 compared to other than lungs.
```{r}
selectFeat(impScores, absVal = F)$orderedFeatMat[1:10,]
```
...
In the tumour niche, we see co-presence of 
...
matrixPlot(pca_res$scores, comp_idx = 1:5, colBy = corMatVec$niche, manualCol = multiSampClustCols, pointSize = 2)
```
...
# Binarise niches
nicheBinary <- as.numeric(corMatVec$niche %in% c("2", "7", "9"))
